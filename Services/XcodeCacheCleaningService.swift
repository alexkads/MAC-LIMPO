...
class XcodeCacheCleaningService: BaseCleaningService, CleaningService {
    ...
    func scan(progress: ((String) -> Void)?) async -> ScanResult {
        ...
        logger.log(
            "Escaneamento concluído: \(fileHelper.formatBytes(totalSize)) em \(items.count) categorias",
            level: .info
        )
    ...
    private func scanOldSimulators() async -> (size: Int64, description: String) {
        ...
        if oldSimsCount > 0 {
            return (
                oldSimsSize,
                "Simuladores antigos (\(oldSimsCount) dispositivos): \(fileHelper.formatBytes(oldSimsSize))"
            )
        }
    ...
    func clean() async -> CleaningResult {
        ...
        logger.log(
            "Limpeza concluída: \(fileHelper.formatBytes(bytesRemoved)) removidos, \(filesRemoved) arquivos",
            level: .info
        )
    ...
    private func cleanSwiftPMBuildArtifacts() async -> (bytesRemoved: Int64, filesRemoved: Int, errors: [String]) {
        ...
        for searchPath in searchPaths {
            ...
            let result = ShellExecutor.shared.execute(findCommand)
