...
class MessagingAppsCleaningService: BaseCleaningService, CleaningService {
    ...
    func clean() async -> CleaningResult {
            ...
            for path in paths {
                ...
                if fileHelper.fileExists(atPath: expandedPath) {
                    ...
                    for item in contents {
                        ...
                        let itemSize = fileHelper.sizeOfDirectory(atPath: itemPath)
                        do {
                    ...
                    if pathFreed > 0 {
                        ...
                        logger.log("Limpou \(app): \(fileHelper.formatBytes(pathFreed))", level: .debug)
                    }
                }
            }
        }
        ...
        logger.log(
            "Limpeza de apps de mensagens concluída: \(fileHelper.formatBytes(bytesRemoved)) liberados",
            level: .info
        )
