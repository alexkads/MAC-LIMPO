...
class TempFilesCleaningService: BaseCleaningService, CleaningService {
    ...
    func scan(progress _: ((String) -> Void)?) async -> ScanResult {
        ...
        Logger.shared.scan(
            category: "TempFiles",
            message: "Scan complete: \(fileHelper.formatBytes(totalSize)) in \(items.count) locations"
        )
    ...
    func clean() async -> CleaningResult {
        ...
        if success {
            Logger.shared
                .success(
                    "TempFiles cleanup complete: \(fileHelper.formatBytes(bytesRemoved)), \(filesRemoved) files, \(errors.count) errors"
                )
        } else {
    ...
    private func cleanTmpDirectory(bytesRemoved: inout Int64, filesRemoved: inout Int, errors _: inout [String]) async {
        let tmpPath = "/tmp"
