...
class AdobeCleaningService: BaseCleaningService, CleaningService {
    ...
    private func resolvePaths(_ path: String) -> [String] {
        ...
        if path.contains("*") {
            ...
            for item in contents {
                if item
                    .range(of: pattern.replacingOccurrences(of: "*", with: ".*"), options: .regularExpression) != nil
                {
    ...
    func clean() async -> CleaningResult {
        ...
        logger.log(
            "Limpeza concluída: \(fileHelper.formatBytes(bytesRemoved)) removidos em \(String(format: "%.2f", executionTime))s",
            level: .info
        )
